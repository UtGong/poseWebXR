...
{
    public class BarChartDual : MonoBehaviour
    {
        public List<float> dataValues1 = new List<float>(); // Simulate your dataValue
        public List<float> dataValues2 = new List<float>(); // Simulate your dataValue
        public RectTransform barPrefab;
        public RectTransform chartContainer1; // Chart container for dataset 1
        public RectTransform chartContainer2; // Chart container for dataset 2
        public RectTransform AxisContainer;
        public float barSpacing = 10f;
        public Color colorGroup1 = Color.white;
        public Color colorGroup2 = Color.white;
        ...
        {
            CreateBarChart(dataValues1, colorGroup1, chartContainer1);
            CreateBarChart(dataValues2, colorGroup2, chartContainer2);
            GameObject.Find("Group1").GetComponent<Image>().color = colorGroup1;
            GameObject.Find("Group2").GetComponent<Image>().color = colorGroup2;

        }

        private void CreateBarChart(List<float> dataValues, Color barColor, RectTransform chartContainer)
        {
        ...
        private void OnPointerEnterBar(PointerEventData eventData, RectTransform bar)
        {
            ...
            // Determine which chart container the bar belongs to and use the corresponding dataValues
            if (bar.parent == chartContainer1)
            {
                chartContainer = chartContainer1;
                dataValues = dataValues1;
            }
            else if (bar.parent == chartContainer2)
            {
            ...
            {
                return; // Exit the function if the bar does not belong to any chart container
            }

            float barValue = bar.GetComponent<RectTransform>().sizeDelta.y / chartContainer.rect.height * Mathf.Max(dataValues.ToArray());
            dataText.text = "Data: " + barValue;
            dataText.gameObject.SetActive(true);

            // Highlight the bar on hover by changing its color
        ...
        private void OnPointerExitBar(PointerEventData eventData, RectTransform bar)
        {
            ...
            {
                chartContainer = chartContainer1;
                bar.GetComponent<Image>().color = colorGroup1;

            }
            else if (bar.parent == chartContainer2)
            {
                chartContainer = chartContainer2;
                bar.GetComponent<Image>().color = colorGroup2;
            }
            else
            {
                return; // Exit the function if the bar does not belong to any chart container
