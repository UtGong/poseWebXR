...
public class CameraManager : MonoBehaviour
{
        ...
        if (mainModel != null && additionalModel != null)
        {
            ...
            foreach (var cameraSet in cameraSets)
            {
                ...
                if (mainCamera != null && additionalCamera != null)
                {
                    ...
                    Transform mainHipsTransform = mainModel.transform.Find("Armature/mixamorig:Hips");
                    Transform additionalHipsTransform = additionalModel.transform.Find("Armature/mixamorig:Hips");
    ...
    public void CreateCameras()
    {
        ...
        mainCameraObj.transform.parent = mainModel.transform.parent;

        // <-- NEW: Generate camera position based on mainModel's Armature/Hips for the first 4 sets -->
        if (mainModel != null && cameraSetCounter < 4)
        {
            Transform armature = mainModel.transform.Find("Armature");
            Transform mainHips = armature != null
                ? armature.GetComponentsInChildren<Transform>().FirstOrDefault(t => t.name.Contains("Hips"))
                : null;
            if (mainHips != null)
            {
                Vector3 hipsPos = mainHips.position;
                float distance = 2.0f;      // Adjust as needed
                float verticalOffset = 1.0f;  // Adjust as needed
                Vector3 computedPosition;
                switch (cameraSetCounter)
                {
                    case 0:
                        computedPosition = hipsPos + new Vector3(distance, verticalOffset, 0);
                        break;
                    case 1:
                        computedPosition = hipsPos + new Vector3(-distance, verticalOffset, 0);
                        break;
                    case 2:
                        computedPosition = hipsPos + new Vector3(0, verticalOffset, distance);
                        break;
                    case 3:
                        computedPosition = hipsPos + new Vector3(0, verticalOffset, -distance);
                        break;
                ...
                // Make the main camera look toward the hips
                Quaternion computedRotation = Quaternion.LookRotation(hipsPos - computedPosition, Vector3.up);
                mainCameraObj.transform.position = computedPosition;
                mainCameraObj.transform.rotation = computedRotation;
                Debug.Log("Main camera positioned at: " + computedPosition + " with rotation: " + computedRotation);
            }
            else
            {
                Debug.LogWarning("Hips transform not found on main model.");
            }
        }
        // <-- END NEW -->
        ...
        additionalCameraObj.transform.parent = additionalModel.transform.parent;
        ...
        // NEW: Adjust the additional camera relative to the Armature/Hips
        if (mainModel != null && additionalModel != null)
        {
            Transform mainArmature = mainModel.transform.Find("Armature");
            Transform additionalArmature = additionalModel.transform.Find("Armature");
            if (mainArmature != null && additionalArmature != null)
            {
                Transform mainArmatureHips = mainArmature.GetComponentsInChildren<Transform>().FirstOrDefault(t => t.name.Contains("Hips"));
                Transform additionalArmatureHips = additionalArmature.GetComponentsInChildren<Transform>().FirstOrDefault(t => t.name.Contains("Hips"));
                if (mainArmatureHips != null && additionalArmatureHips != null)
                {
                    // Calculate offset (distance and direction) from mainArmatureHips to main camera
                    Vector3 offset = mainCameraObj.transform.position - mainArmatureHips.position;
                    // Set additional camera's position relative to the additional model's Armature/Hips using the same offset
                    additionalCameraObj.transform.position = additionalArmatureHips.position + offset;
                    // Match the main camera's rotation
                    additionalCameraObj.transform.rotation = mainCameraObj.transform.rotation;
                    Debug.Log("Additional camera positioned at: " + additionalCameraObj.transform.position);
                }
                else
                {
                    Debug.LogWarning("Armature Hips not found on one or both models.");
                }
            }
            else
            {
                Debug.LogWarning("Armature not found on one or both models.");
            }
        }
